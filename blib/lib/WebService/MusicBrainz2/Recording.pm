package WebService::MusicBrainz2::Recording;
...
sub _init {
	...
	$q->set_inc_params(
		qw/artists releases discids media puids isrcs artist-credits aliases
		tags ratings user-tags user-ratings artist-rels label-rels
		recording-rels release-rels release-group-rels url-rels work-rels
		work-level-rels/
	);

	$q->set_browse_inc_params(
		qw/puids isrcs artist-credits tags ratings user-tags user-ratings
...
sub search {
	...
	my $response = $self->query->search('recording', $params);    
...
sub lookup {
	...
	my $response = $self->query->lookup('recording', $mbid, $incs);
...
sub browse {
	...
	my $response = $self->query->browse('recording', $params);
